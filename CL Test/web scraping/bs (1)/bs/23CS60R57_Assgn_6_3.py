...
def get_row():
    ...
    def scrap(text):
        ...
        #print(n_sport)
        find_ranks(url)
        r1,r2,r3,country_list,sport_name=None,None,None,None,None
        ...
        query = "CREATE TABLE IF NOT EXISTS Olympics(Name, URL, Year, Host,Nations,Athletes,Sports,R1,R2,R3,DoneORNotDone)"
        cursor.execute(query)
        c_list=None
        c_sports=None
        query = "INSERT INTO Olympics VALUES ('%s','%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s','%s')"%(title, url,year,host, c_list,n_athlete , c_sports,r1,r2,r3,0)
        cursor.execute(query)
        con.commit()
        cursor.close()
