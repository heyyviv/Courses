...
void thread(char** argv,int signal){
	int code=0;
...
void execute(char** argv){
    ...
    if(strcmp(argv[0],"addvec")==0 || strcmp(argv[0],"subvec")==0 || strcmp(argv[0],"dotprod")==0){
        //printf("thread");
        if(last_index==3){thread(argv,0);
        }else{thread(argv,1);}
        
        return;

    }
    if(strcmp(argv[0],"vi")==0){  //code editor
        ...
        pid=fork();
        if(pid==0){  // child excute this 
            execlp("./text_editor","text_editor",argv[1],NULL);
            exit(1);

        }else{
            while(wait(&status)!=pid){   //parent wait here
            }
        }
        return ;
    }
    ...
    if(pid==0){
        ...
        /*
        char *programName = "ls";
        char *arguments[] = {"ls", "-l", "/home/user", NULL};
...
int main(){
    ...
    while(1){
        ...
        is_pipe=0;
        in_background=0;
        char *argv[100];// array of char*
        //empty input
        prev = readline("Existence>>");   // readline user input
        ...
        strcat(line,prev);  // concat string 
        //printf("%s",line);
        ...
        add_history(line);   // adding history in readline

        size_t l=strlen(line);     //  length of string  
        //line[l-1]='\0';//making last next line as NULL
        check_pipe(line);   // checking | contain in user input 
